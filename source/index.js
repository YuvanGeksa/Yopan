...
export function initApp() {
  ...
  // Apply username if already saved, otherwise ask once
  const savedUsername = getSavedUsername();
  if (savedUsername) {
    applyUsernameToInputs(savedUsername);
  } else {
    openUsernameOverlay();
  }
  ...
  uSkip?.addEventListener("click", () => closeUsernameOverlay());
...
try {
  ...
  $all(".buyBtn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const username = String(meta.name || localStorage.getItem("username") || "").trim();
      if (!username) {
      ...
      const username = String(meta.name || localStorage.getItem("username") || "").trim();
      if (!username) {
        // force username gate if missing
        document.getElementById("usernameOverlay")?.classList.remove("hidden");
        toast("Isi username dulu ya bro.", "error");
        return;
      }
